...
namespace LeagueSandbox.GameServer.GameObjects
{
    class Pathfinder
    {
        ...
        public static Path getPath(Vector3 from, Vector3 to, float boxSize)
        {
            var path = new Path();
            var job = new PathJob();
        ...
        public static Path getPath(Vector3 from, Vector3 to)// { if (!chart->getAIMesh()) CORE_FATAL("Can't get path because of a missing AIMesh."); return getPath(from, to, PATH_DEFAULT_BOX_SIZE(mesh->getSize())); }
        {
    ...
    class PathJob
    {
        ...
        public Vector3 fromGridToPosition(Vector3 position)
        {
        ...
        public Vector3 fromPositionToGrid(Vector3 position)
        {
        ...
        public List<Vector3> reconstructPath()
        {
            ...
            foreach (var v in i)
                ret.Add(fromGridToPosition(new Vector3(v.x, v.y, 0)));

            return ret;
        }

        public List<Vector3> reconstructUnfinishedPath()
        {
            List<Vector3> ret = new List<Vector3>();
            ...
            foreach (var i in a)
                ret.Add(fromGridToPosition(new Vector3(i.x, i.y, 0)));
                        ...
                        for (int dy = -1; dy <= 1; dy++)
                        {
                            if (!(dx == 0 && dy == 0)) // in all 8 directions, ignore the x==y==0 where we dont move
                            {
                                if (!isGridNodeOccupied(currentNode.x + dx, currentNode.y + dy)) // Is something here?
                                {
                                    ...
                                    {
                                        addToOpenList(new Vector3(currentNode.x + dx, currentNode.y + dy, 0), currentNode);
                                    }
        ...
        public void addRealPosToOpenList(Vector3 position, PathNode parent)
        {
            addGridPosToOpenList(fromPositionToGrid(position), parent);
        }

        public void addGridPosToOpenList(Vector3 position, PathNode parent)
        {
        ...
        public void addToOpenList(Vector3 position, PathNode parent)
        {
        ...
        public void insertObstructions(AIMesh mesh)
        {
            /*
            if (mesh != null)
            {
                ...
                if (mesh.isLoaded()) // if we have loaded the mesh
                    for (int x = 0; x < GRID_WIDTH; x++) // for every grid piece
                        ...
                        {
                            Vector3 translated = fromGridToPosition(new Vector3(x, y, 0));
            ...
            if (chart != null)
            {
                ...
                {
                    if (!(i.Value is Obj_AI_Minion) && !(i.Value is Obj_AI_Hero))
                        continue;

                    Vector3 gridPos = fromPositionToGrid(i.Value.Position); // get the position in grid size
  ...
  #define MAX(a,b) (((a)>(b))?(a):(b))
    ...
    class PathNode
    {
        ...
        public PathNode(Vector3 pos, int ag, int ah, PathNode p)
        {
